...
async def on_message(message):
  ...
  c = 0
  while type(message) == discord.PartialMessage:
  ...
  if msg.startswith("T!neko"):
    try:
      resp = requests.get("https://nekos.best/api/v2/neko").json()
      resp = resp["results"][0]["url"]

      emb = discord.Embed(title=str(message.author), colour=MAIN)
      emb.set_image(url=resp)
      await message.channel.send(embed=emb)
    except:
      await error(message)
  ...
  if "slay" in msg.lower():
    await message.channel.send(random.choice(var.insults))
      


keep_alive()
client.run(os.getenv("TIGERR_KEY"))